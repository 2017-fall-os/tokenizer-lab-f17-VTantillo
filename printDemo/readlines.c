...
int countLines(int fd, int *pMaxLineLen) {
    char buf[BUFLEN];   // beginning of buffer
    char *pBuf;         // buffer pointer
    char *pBufLimit;    // end of buffer
    ...
    while(numBytesRead = read(fd, &buf, BUFLEN)) {
        ...
        // immediately following last byte read
        pBufLimit = buf + numBytesRead;

        // ...every character in buf
        for (pBuf = buf; pBuf != pBufLimit; pBuf++) {
            lastChar = *pBuf;
            if (lastChar == '\n') {
                numLines++;
        ...
        while(numBytesRead = read(fd, &readBuf, BUFLEN)) {
            ...
            for (pReadBuf = readBuf; pReadBuf != pReadBufLimit; pReadBuf++) {
                ...
                if (lastChar == '\n') {	// line terminator found
                    // terminate copy in buffer
                    *pLineBuf = 0;

                    //insert copy into lines[]
                    *(pLines++) = strcopy(lineBuf);

                    // reset lineBuf before reading next line
                    pLineBuf = lineBuf;
